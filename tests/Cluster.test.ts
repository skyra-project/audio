...
gateway.on('MESSAGE_CREATE', async (shard, m) => {
	...
	if (m.content === 'decode') {
		const trackResponse = await cluster
			.get('281630801660215296')
			.node.load('https://www.youtube.com/playlist?list=PLe8jmEHFkvsaDOOWcREvkgFoj6MD0pQ67');
	...
	if (m.content === 'play') {
		const trackResponse = await cluster
			.get('281630801660215296')
			.node.load('https://www.youtube.com/playlist?list=PLe8jmEHFkvsaDOOWcREvkgFoj6MD0pQ67');
		void cluster.get('281630801660215296').play(trackResponse.tracks[0]);
	}
